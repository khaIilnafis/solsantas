...
import { getEnumKeyByEnumValue, okToFailAsync } from '../common/util';
...
async function getHolderByMint(mint: PublicKey, connection: Connection): Promise<PublicKey> {
...
async function metadatasToTokens(rawMetadatas: any[], connection: Connection): Promise<IToken[]> {
  const promises = await Promise.all(
    rawMetadatas.map(async (m) => {
      try {
        const metadata = deserializeMetadata(m);
        const mint = new PublicKey(metadata.data.mint);
        const address = await getHolderByMint(mint, connection);
        return {
          mint,
          address,
...
async function getTokensByCreator(creator: PublicKey, connection: Connection): Promise<IToken[]> {
  ...
  return metadatasToTokens(rawMetadatas, connection);
}

async function getTokensByUpdateAuthority(updateAuthority: PublicKey, connection: Connection): Promise<IToken[]> {
  const rawMetadatas = await connection.getProgramAccounts(
    new PublicKey(METADATA_PROGRAM_ID),
    {
      filters: [
...
async function getTokensByMint(mint: PublicKey, connection: Connection): Promise<IToken[]> {
  return [
    {
      mint,
      address: await getHolderByMint(mint, connection),
    },
  ];
}

async function getTokensByOwner(owner: PublicKey, connection: Connection): Promise<IToken[]> {
...
async function getEditionInfoByMint(mint: PublicKey, connection: Connection) {
  // untriaged
...
async function getParentEdition(editionData: EditionData, connection:Connection) {
  const masterEditionPDA = new PublicKey(editionData.parent);
  const masterInfo = await Account.getInfo(connection, masterEditionPDA);
  ...
  return Promise.all(
    tokens.map(async (t) =>
      ...
      ({
        ...
        ...(await okToFailAsync(getEditionInfoByMint, [t.mint, connection], true)),
      })
    )
  );
}
